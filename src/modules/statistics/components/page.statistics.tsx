									...
									<label>
										...
										<TagComponent
											text={
												...
												round(
													statistics.totalPayments + expenses.totalPayments
												).toString()
											}
									...
									<label>
										...
										<TagComponent
											text={
												...
												round(
													statistics.totalExpenses + expenses.totalExpenses
												).toString()
											}
											type={TagType.danger}
										/>
						...
						<Row gutter={24}>
							<Col md={8}>
								...
								<SectionComponent title={'Procedures'} image={'EditContact'} >
									<h3> <b> Total :</b>  {
									statistics.selectedAppointments.filter((item, i) => item.patientID !== "" && item.procedureId !== "").length}</h3>
									<PieChartComponent
										height={400}
										{...{
											data: [
												...
												{ label: text("Missed: " + this.calculatePercentileProcedure('Missed')), value: this.calculatePercentileProcedure('Delayed') },
									...
									<PieChartComponent
										...
										{...{
											data: [
												...
												{ label: text("Checked In: " + this.calculatePercentileVisits('CheckedIn')), value: this.calculatePercentileVisits('CheckedIn') },
												{ label: text("In Progress: " + this.calculatePercentileVisits('InProgress')), value: this.calculatePercentileVisits('InProgress') },
							...
							<Col md={8}>
								...
								<SectionComponent title={'Financials'} image={'Money'} >
									...
									<p> <b style={{ fontSize: '20px' }}> Total Income:  </b> <b style={{ fontSize: '14px' }}>  {setting.getSetting("currencySymbol")}  {this.calculateFinancialProfit()} </b>   </p>
								...
								<SectionComponent title={'Top Dentist'} image={'PeopleAdd'} >
									...
									<table className="ms-table" >
										...
										<tbody>
											...
											<tr> 
												<td> { this.calculateTopDentistVisits() ? this.calculateTopDentistVisits().dentist : "No dentist" } </td>  <td>{this.calculateTopDentistProceduresCount( this.calculateTopDentistProcedures().procedureId)}</td>  <td> { this.calculateTopDentistVisits() ? this.calculateTopDentistVisitsCount( this.calculateTopDentistVisits()._id ) : "Nil" } </td>
											</tr>
											)}
										</tbody>
									</table>
