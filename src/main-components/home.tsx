...
import { appointments, setting, patients, colors, settingsNamespace, Appointment, Patient, AppointmentsList, procedureList, procedures, staff } from "@modules";
import { computed, observable } from "mobx";
import { appointmentsByDateChart } from "./line-chart";
...
export class HomeView extends React.Component<{}, MyState> {
  ...
  constructor(props: {}) {
    ...
    this.state = {
      ...
      patientid: "",
      selectappoint: "",
      globalApointment: null
    };
  }

  componentDidUpdate(prevProps, prevState) {
    if (prevState.pageview !== this.state.pageview) {
      console.log('pageview state has changed.!!!!!!!!!')
    }
  }
  ...
  //private selectappoint: Appointment = undefined;
  ...
  get todayAppointments() {
    ...
    //   this.time.day
    // );
    const result = appointments.appointmentsForDay(new Date().getTime(), 0, 0);
  ...
  get values() {
		/*return patients.list
			.map(x => x.datex)
			.reduce(( result: { label: string; value: number }[], datex) => {
  ...
  getTodayPatients = () => {
      ...
      let filteredList = patients.list.filter((item: any) => {
        let date = new Date(Number(item.datex));
        return (
  ...
  calculatePercentileProcedure(status: String) {
    ...
    this.todayAppointments
      .forEach((item, i) =>  {
        // lorenzo item.patientID !== "" && item.procedureId !== "" &&
          item.patient.procedures.forEach(procedure => {
            //console.log("buscamos!!!: " + status);
            //console.log("consultamos!!!: " + procedure.status);

            if (procedure.status === status) {
              count++;
              console.log("encontre!!! y contador es = "+ count);
            }
      })
    }
      )
  ...
  calculatePercentileVisits(status: String) {
    return appointments.list
      .filter((item, i) => item.patientID !== "" && item.status === status
      ).length;
  }
  calculatePercentileVisitsTime(status: String) {
    let count = 0;
    appointments.list.filter((item, i) => {
      //console.log("buscamos!!!: " + status);
      //console.log("consultamos!!!: " + item.status);
     if ( item.patientID !== "" && new Date(item.date).toDateString() === new Date().toDateString()  && item.status.trim().toLowerCase() === status.trim().toLowerCase()){
            count++;
           // console.log("SI ENCONTRE ES!!!!: " + status + ' y contador es : ' + count);
          }
        }
    );
   ...
   // return count;
    ...
    // this.todayAppointments
    //   .filter((item, i) =>
    //       // lorenzo item.patientID !== "" &&
    //       // lorenzo item.status === status).length;
    //
    //       item.patient.procedures.forEach(procedure => {
    //         if (procedure.status === status) {
    //           count++;
    ...
    //         count++;
    //       }
    //       if ( item.patientID !== "" && item.status.trim().toLowerCase() === status.trim().toLowerCase()){
    //         count++;
    //         console.log("SI ENCONTRE ES!!!!: " + status + ' y contador es : ' + count);
    //       }
    //           // item.patient.procedures.forEach(procedure => {
  ...
  calculateTodayAppointment = (date: number) => {
    const result = appointments.appointmentsForDay(date, 0, 0);

    //this.setState({globalApointment: result});
    //globalApointment
  ...
  totalProcedures = () => {
    ...
    this.todayAppointments
      .forEach((item, i) =>  {
       // lorenzo item.patientID !== "" && item.procedureId !== "" &&
        item.patient.procedures.forEach(procedure => {
          // lorenzo if (procedure.id === item.procedureId) {
          if(procedure.status){
            count++;
          }
      ...
      <div className="home p-l-10 p-r-10">
        <div className="container">
          <h2 className="m-b-20">
            ...
            <span style={{ float: 'right' }}>  {this.getDate()} ;  <span id="curTime" style={{ color: 'black' }}></span> </span>
          </h2>
            ...
            <Dropdown
              options={
                [{ id: 'overall', val: 'overall' },
                  ...
                  { id: 'dashboard', val: 'dashboard' }].map((data) => ({
                  key: data.id,
                  text: data.val,
                })) as any
              }
              ...
              defaultSelectedKey={this.state.pageview}
            />
          </div>
          <hr />
          <p>
                      ...
                      <PieChartComponent
                        ...
                        {...{
                          data: [
                            ...
                            { label: text("Delayed: " + this.calculatePercentileVisitsTime('Delayed')), value: this.calculatePercentileVisitsTime('Delayed') },
                            { label: text("Checked In: " + this.calculatePercentileVisitsTime('CheckedIn')), value: this.calculatePercentileVisitsTime('CheckedIn') },
              ...
              <Row>
                <Col md={8}>
                  ...
                  >
                    <SectionComponent title={'Patients'} image={'PeopleAlert'}>
                      ...
                      />
                        {/*{String(this.calculateTodayAppointment(new Date().getTime()).length)}*/}
                ...
                <Col md={8}>
                  ...
                  >
                    <SectionComponent title={'Procedures'} image={'EditContact'}>
                      <h3> <b> Total :</b> {this.totalProcedures()}</h3>
                      <PieChartComponent
                        height={400}
                        {...{
                          data: [
                ...
                <Col md={8}>
                  ...
                  <SectionComponent title={'Visits'} image={'CalendarMirrored'}>
                    <h3>
                      <b> Total :</b>
                      {appointments.list.filter((item, i) => item.patientID !== "" && new Date(item.date).toDateString() === new Date().toDateString()).length}
                    </h3>
                    <PieChartComponent
                      height={400}
                      {...{
                  ...
                  >
                    <SectionComponent title={'Room performance'} image={'CalendarWorkWeek'}>
                      ...
                      <table className="ms-table">
                        ...
                        <tbody>
                          <tr>
                            <td>Name</td><td> Room:{this.calculateRoomsHigh().roomNumber}</td><td> Room: {this.calculateRoomsLow().roomNumber}</td>
                          </tr>
                          <tr>
                            <td>Visits</td><td>{this.calculateRoomsCount(this.calculateRoomsHigh().roomNumber)} </td><td> {this.calculateRoomsCount(this.calculateRoomsLow().roomNumber)}</td>
                          </tr>
                          <tr>
                            <td>Payments</td><td>{setting.getSetting("currencySymbol")} {this.calculateRoomsPayments(this.calculateRoomsHigh().roomNumber)}</td><td>{setting.getSetting("currencySymbol")}{this.calculateRoomsPayments(this.calculateRoomsLow().roomNumber)}</td>
                          </tr>
                        </tbody>
                      </table>
                    ...
                    <SectionComponent title={'Patient Information'} image={'PublicCalendar'}>
                      ...
                      <div style={{ display: "flex", flex: 1, flexDirection: "row", width: "100%", justifyContent: "space-around", textAlign: "center", fontSize: "14px", fontWeight: "bold", marginTop: "30px", }}>
                        {this.state.selectappoint !== "" && <>
                          ...
                          <div><div>Total Paid       </div> <div className="mt-2 font-weight-light">{patientDetails.paymentDetails.totalPaid}</div></div>
                          ...
                          {this.state.selectappoint !== "" ? (
                            <tbody>
                              <tr> <td> <b> Last Appointment </b> </td> <td>{patientDetails.lastAppointment
                                ...
                                ? patientDetails.lastAppointment.treatment.type
                                : "" : ""}</td>
                              </tr>
                              <tr> <td> <b> Insaurance </b> </td> <td> {patientDetails.insurance && patientDetails.insurance.name}  </td> </tr>
                              <tr> <td> <b> Note </b> </td> <td> {patientDetails.medicalNote}  </td> </tr>
